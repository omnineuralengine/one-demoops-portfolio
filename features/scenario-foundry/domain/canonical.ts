...
export function canonicalStringify(value: unknown): string {
  ...
  function encode(current: unknown): string {
    ...
    if (typeof current === "number") {
      if (!Number.isFinite(current)) throw new Error("CANONICAL_UNSUPPORTED_VALUE");
      return JSON.stringify(Object.is(current, -0) ? 0 : current);
    }
    if (typeof current !== "object") throw new Error("CANONICAL_UNSUPPORTED_VALUE");
    if (ancestors.has(current)) throw new Error("CANONICAL_CYCLE");
    ...
    try {
      ...
      try {
        ...
        throw new Error("CANONICAL_EXOTIC_OBJECT");
      }
      if (symbols.length > 0) throw new Error("CANONICAL_UNSUPPORTED_VALUE");
      if (Object.values(descriptors).some((descriptor) => descriptor.get || descriptor.set || !("value" in descriptor))) {
        throw new Error("CANONICAL_EXOTIC_OBJECT");
      }
      if (Array.isArray(current)) {
        const keys = Object.keys(current);
        if (keys.length !== current.length || keys.some((key, index) => key !== String(index))) throw new Error("CANONICAL_UNSUPPORTED_VALUE");
        return `[${keys.map((key) => encode(descriptors[key].value)).join(",")}]`;
      }
      let prototype: object | null;
      try { prototype = Object.getPrototypeOf(current); }
      catch { throw new Error("CANONICAL_EXOTIC_OBJECT"); }
      if (prototype !== Object.prototype && prototype !== null) throw new Error("CANONICAL_EXOTIC_OBJECT");
...
/** Browser-safe synchronous SHA-256. Tests cross-check it against Node's implementation. */
...
export function stableId(prefix: string, value: unknown): string {
  return `${prefix}:${stableHash(value).slice(0, 32)}`;
}
